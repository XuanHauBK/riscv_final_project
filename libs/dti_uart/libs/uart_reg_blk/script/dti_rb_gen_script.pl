...
#------------------------------------------------------------------------#
  ...
  sub rbRegisterGen (@) {
      ...
      for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
        ...
        for ( my $field_index = 1; $field_index <= $#reg_field; $field_index++ ) {
          ...
          if ( $reg_field[$field_index]{'field_hwacc'} =~ /R(O|W)/ ) {
            if ($signal_name =~ /ctrl_start_tx/) {
              print $rb_regs_fh "  ", "input                 ctrl_reg_update,                     // Ctrl register update", "\n";
            }
            if ($reg_array[$reg_index]{'reg_name'} =~ /NA/) {
    ...
    print $rb_regs_fh "  ", "input";        printSpace(17, $rb_regs_fh);                                                                          print $rb_regs_fh "reset,";
    printSpace(36-length("reset"), $rb_regs_fh);
    print $rb_regs_fh "// ", "Synchronous Reset, Active LOW", "\n";
    
    # Read Write Control Signals
    print $rb_regs_fh "// Read Write Control Signals", "\n";
    print $rb_regs_fh "  ", "input";        printSpace( 8, $rb_regs_fh); print $rb_regs_fh "[11:0]"; printSpace(9-length("[11:0]"), $rb_regs_fh); print $rb_regs_fh "waddr,";
    printSpace(36-length("waddr"), $rb_regs_fh);
    print $rb_regs_fh "// ", "Write Address", "\n";
    print $rb_regs_fh "  ", "input";        printSpace( 8, $rb_regs_fh); print $rb_regs_fh "[11:0]"; printSpace(9-length("[11:0]"), $rb_regs_fh); print $rb_regs_fh "raddr,";
    ...
    print $rb_regs_fh "  ", "input";        printSpace(17, $rb_regs_fh);                                                                          print $rb_regs_fh "wr_en,";
    printSpace(36-length("wr_en"), $rb_regs_fh);
    print $rb_regs_fh "// ", "Write Enable", "\n";
    print $rb_regs_fh "  ", "input";        printSpace(17, $rb_regs_fh);                                                                          print $rb_regs_fh "rd_en,";
    printSpace(36-length("rd_en"), $rb_regs_fh);
    print $rb_regs_fh "// ", "Read Enable", "\n";
    ...
    print $rb_regs_fh "// ", "Write Address Decode Status", "\n";
    print $rb_regs_fh "  ", "output logic"; printSpace(10, $rb_regs_fh);                                                                          print $rb_regs_fh "raddrerr";
    printSpace(36-length("raddrerr"), $rb_regs_fh);
    print $rb_regs_fh "// ", "Read Address Decode Status", "\n";
    ...
    for ( my $reg_index = 1; $reg_index <= $#reg_array; $reg_index++ ) {
      ...
      for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
        ...
        if ( $reg_array[$reg_index]{'reg_access'} =~ /RW/ ) {
          ...
          print $rb_regs_fh "= ";
          print $rb_regs_fh "rd_en & (raddr == 32'h"."$inst_addr_hex".");\n";
          ...
          printSpace(20-length($en_name), $rb_regs_fh);
          print $rb_regs_fh "= ";
          if ( $reg_array[$reg_index]{'reg_name'} =~ /uci/ ) {
            print $rb_regs_fh "wr_en & (waddr == 32'h"."$inst_addr_hex".") & user_cmd_ready;\n";
          }
          else {
            print $rb_regs_fh "wr_en & (waddr == 32'h"."$inst_addr_hex".");\n";
          }
        }
        if ( $reg_array[$reg_index]{'reg_access'} =~ /RO/ ) {
          # Read Enable
    ...
    print $rb_regs_fh "  ", "wire waddrerr_nxt;", "\n";
    print $rb_regs_fh "  ", "assign waddrerr_nxt = ~|{", "\n";
    for ( my $reg_index = 1; $reg_index <= $#reg_array; $reg_index++ ) {
      my $inst_no   = $reg_array[$reg_index]{'num_inst'};
      my $inst_suffix = $reg_array[$reg_index]{'suffix'};
      for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
        my $name_ext    = ( $inst_no > 1 ) ? "_"."$inst_suffix"."$inst_index" : "";
        if ( $reg_array[$reg_index]{'reg_access'} =~ /RW/ ) {
          #if ( ($reg_array[$reg_index+1]{'reg_access'} !~ /RW/) && ($inst_index == $inst_no - 1) ) {
          # if (($reg_array[$reg_index]{'reg_name'} =~ /rtgc/) && ($name_ext =~ /ch1/)) {
          if (($reg_array[$reg_index+1]{'reg_access'} !~ /RW/) ) {
            printSpace(28, $rb_regs_fh); print $rb_regs_fh "$reg_array[$reg_index]{'reg_name'}"."$name_ext"."_wen"."\n";
          }
          else {
            printSpace(28, $rb_regs_fh); print $rb_regs_fh "$reg_array[$reg_index]{'reg_name'}"."$name_ext"."_wen".",\n";
          }
    ...
    printSpace(26, $rb_regs_fh); print $rb_regs_fh "};\n";

    print $rb_regs_fh "\n";
    print $rb_regs_fh "  ", "always_ff @(posedge clk) begin : WRITE_ADDR_DECODE_STATUS_PROC\n";
    print $rb_regs_fh "  ", "  if (!reset) begin\n";
    print $rb_regs_fh "  ", "    wack     <= 1'b0;\n";
    print $rb_regs_fh "  ", "    waddrerr <= 1'b0;\n";
    print $rb_regs_fh "  ", "  end\n";
    print $rb_regs_fh "  ", "  else begin\n";
    print $rb_regs_fh "  ", "     if (wr_en) begin\n";
    print $rb_regs_fh "  ", "       wack     <= 1'b1;\n";
    print $rb_regs_fh "  ", "       waddrerr <= waddrerr_nxt;\n";
    print $rb_regs_fh "  ", "     end\n";
    print $rb_regs_fh "  ", "     else begin\n";
    print $rb_regs_fh "  ", "       wack     <= 1'b0;\n";
    print $rb_regs_fh "  ", "       waddrerr <= 1'b0;\n";
    ...
    print $rb_regs_fh "\n";
    print $rb_regs_fh "  ", "always_comb begin : READ_ADDR_DECODE_STATUS_PROC\n";
    print $rb_regs_fh "  ", "  if (!rd_en) begin\n";
    ...
    for ( my $reg_index = 1; $reg_index <= $#reg_array; $reg_index++ ) {
      ...
      for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
        ...
        if(not $reg_array[$reg_index]{'reg_name'} =~ /^NA$/ ){
          print $rb_regs_fh "//-----------------------------------------------------------------------------\n";
          ...
          for ( my $field_index = 1; $field_index <= $#reg_field; $field_index++ ) {
            ...
            if ($reg_field[$field_index]{'field_swacc'} =~ /RW/ ) {
              ...
              if ($reg_field_name =~ /ctrl_start_tx/) {
                print $rb_regs_fh "  ", "always_ff @(posedge clk) begin : ", "$proc_name\n";
                print $rb_regs_fh "  ", "  if (!reset) begin", "\n";
                ...
                print $rb_regs_fh "  ", "  end", "\n";
                print $rb_regs_fh "  ", "  else if (ctrl_reg_update) begin", "\n";
                print $rb_regs_fh "  ", "    ", "$reg_field_name", " <= 0", ";\n";
                print $rb_regs_fh "  ", "  end", "\n";
                print $rb_regs_fh "  ", "end", "\n";
              }
              else {
    ...
    for ( my $field_index = 1; $field_index <= $#reg_field; $field_index++ ) {
      for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
        ...
        # print $rb_regs_fh "= cfg_int_en"." & "."stt"."$name_ext"."_"."$reg_field[$field_index]{'field_name'}"."_reg".";\n";
        print $rb_regs_fh "= "."stt"."$name_ext"."_"."$reg_field[$field_index]{'field_name'}"."_reg".";\n";
      }
    }
    print $rb_regs_fh "\n", "endmodule";
...
#------------------------------------------------------------------------#
# rbDerivativeRegGen Function
#------------------------------------------------------------------------#
  # Generate rb_derivative_reg.sv, Derivative Registers
  # Parameters:
  #   $_[0]: Register Definition Array
  # Return:
  #   None
  
  sub rbDerivativeRegGen (@) {
    my @reg_def_array    = @{$_[0]};
    
    my $rb_derivative_reg_filename = "..\/hdl\/rb_derivative_reg.sv";
    open (my $rb_derivative_reg_fh, ">$rb_derivative_reg_filename") || die "Can't open new file: $! $rb_derivative_reg_filename\n";
    ...
    # Header
    printHeader($rb_derivative_reg_fh, $rb_derivative_reg_filename, "Derivative Registers");
    
    # Module Declaration
    print $rb_derivative_reg_fh "module rb_derivative_reg ( \n";
    # Field Signals
    print $rb_derivative_reg_fh "// Field Signals\n";
    for ( my $reg_index = 1; $reg_index <= $#reg_array; $reg_index++ ) {
      my @reg_field = @{$reg_array[$reg_index]{'reg_field'}};
      my $inst_no   = $reg_array[$reg_index]{'num_inst'};
      my $inst_suffix = $reg_array[$reg_index]{'suffix'};
      for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
        ...
        for ( my $field_index = 1; $field_index <= $#reg_field; $field_index++ ) {
          if ( $reg_field[$field_index]{'field_deriv'} && ($reg_field[$field_index]{'field_deriv'} !~ /no_divide/)) {
            ...
            if ( $reg_field[$field_index]{'field_hwacc'} =~ /R(O|W)/ ) {
              print $rb_derivative_reg_fh "  ","input";
              printSpace(8, $rb_derivative_reg_fh);
              print $rb_derivative_reg_fh "$reg_field_width";
              printSpace(9-length($reg_field_width), $rb_derivative_reg_fh);
              print $rb_derivative_reg_fh "$signal_name".",\n";
            }
            if ( $reg_field[$field_index]{'field_hwacc'} =~ /WO/ ) {
              print $rb_derivative_reg_fh "  ","output wire ";
              printSpace(1, $rb_derivative_reg_fh);
              print $rb_derivative_reg_fh "$reg_field_width";
              printSpace(9-length($reg_field_width), $rb_derivative_reg_fh);
              print $rb_derivative_reg_fh "$signal_name".",\n";
            }
    ...
    print $rb_derivative_reg_fh "// Derivative Signals\n";
    for ( my $deriv_index = 1; $deriv_index <= $#deriv_array; $deriv_index++ ) {
      my $width           = $deriv_array[$deriv_index]{'deriv_width'} - 1;
      my $deriv_width     = ($width > 0) ? "[$width:0]" : "";
      if ( $deriv_array[$deriv_index]{'deriv_io'} eq "Input" ) {
        print $rb_derivative_reg_fh "  ","input";
        printSpace(8, $rb_derivative_reg_fh);
        print $rb_derivative_reg_fh "$deriv_width";
        printSpace(9-length($deriv_width), $rb_derivative_reg_fh);
        if ($deriv_array[$deriv_index]{'deriv_name'} =~ /miso_odis/) {
          print $rb_derivative_reg_fh "$deriv_array[$deriv_index]{'deriv_name'}"."\n";
        }
        else {
          print $rb_derivative_reg_fh "$deriv_array[$deriv_index]{'deriv_name'}".",\n";
        }
      }
      if ( $deriv_array[$deriv_index]{'deriv_io'} eq "Output" ) {
        print $rb_derivative_reg_fh "  ","output wire ";
        printSpace(1, $rb_derivative_reg_fh);
        print $rb_derivative_reg_fh "$deriv_width";
        printSpace(9-length($deriv_width), $rb_derivative_reg_fh);
    ...
    print $rb_derivative_reg_fh "// End\n";
    print $rb_derivative_reg_fh ");\n";
    
    # Assignments
    print $rb_derivative_reg_fh "\n";
    print $rb_derivative_reg_fh "//-------------------------------------------------------------------------\n";
    print $rb_derivative_reg_fh "// Assignments\n";
    print $rb_derivative_reg_fh "//-------------------------------------------------------------------------\n";
    for ( my $deriv_index = 1; $deriv_index <= $#deriv_array; $deriv_index++ ) {
      my @deriv_field = @{$deriv_array[$deriv_index]{'deriv_field'}};
      # Output
      if ( $deriv_array[$deriv_index]{'deriv_io'} eq "Output") {
        print $rb_derivative_reg_fh "  ", "assign ";
        print $rb_derivative_reg_fh "$deriv_array[$deriv_index]{'deriv_name'}";
        printSpace(37-length("  assign $deriv_array[$deriv_index]{'deriv_name'}"), $rb_derivative_reg_fh);
        print $rb_derivative_reg_fh "= {", "\n";
        for ( my $field_index = $#deriv_field; $field_index >= 1; $field_index-- ) {
          printSpace(40, $rb_derivative_reg_fh);
          if ( $field_index == 1 ) {
            print $rb_derivative_reg_fh "$deriv_field[$field_index]{'field_name'}", "\n";
          }
          else {
            print $rb_derivative_reg_fh "$deriv_field[$field_index]{'field_name'}", ",\n";
          }
        }
        printSpace(39, $rb_derivative_reg_fh);
        print $rb_derivative_reg_fh "};\n";
      }
      elsif ( $deriv_array[$deriv_index]{'deriv_io'} eq "Input") {
        print $rb_derivative_reg_fh "  ", "assign {\n";
        for ( my $field_index = $#deriv_field; $field_index >= 1; $field_index-- ) {
          printSpace(2+length("assign {"), $rb_derivative_reg_fh);
          if ( $field_index == 1 ) {
            print $rb_derivative_reg_fh "$deriv_field[$field_index]{'field_name'}", "\n";
          }
          else {
    ...
    print "Generating rb_derivative_reg: Done!\n";
    close($rb_derivative_reg_fh);
  }
...
#  open (my $rb_derivative_reg_fh,  "..\/hdl\/rb_derivative_reg.sv")   || die "Can't open file: $! ..\/hdl\/rb_derivative_reg.sv\n";
    ...
    for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
      ...
      for ( my $field_index = 1; $field_index <= $#reg_field; $field_index++ ) {
        ...
        if ( lc($reg_field[$field_index]{'field_io'}) =~ /input/ ) {
          ...
          printSpace(9-length($reg_field_width), $rb_top_fh);
        ...
        if ( lc($reg_field[$field_index]{'field_io'}) =~ /output/ ) {
          ...
          print $rb_top_fh "$signal_name".",\n";
          if ($signal_name =~ /ctrl_start_tx/) {
            print $rb_top_fh "  ", "input                 ctrl_reg_update", ",\n";
          }
        }
      }
    }
...
#
#  # Derivative Signals
#  print $rb_top_fh "// Derivative Signals\n";
#  my $current_line = "";
#  my $rtl_infile   = $rb_derivative_reg_fh;
#  my $get_line     = 0;
#  
#  seek($rtl_infile,0,0); # Start at the beginning of the file
#  while ($current_line = <$rtl_infile>) {
#    if ( $current_line =~ /Derivative Signals/ ) {
#      $get_line = 1; # Get the lines below
#    }
#    if ( $current_line =~ /End/ ) {
#      $get_line = 0;
#      last;
#    }
#    if ( ($get_line == 1) && ($current_line !~ /Derivative Signals/) ) {
#      $current_line =~ s/\n//g;
#      $current_line =~ s/\,//g;
#      $current_line =~ s/logic/wire /g;
...
#  
  ...
  # Global Signals
  print $rb_top_fh "// Global Signals\n";
  print $rb_top_fh "  ", "input";        printSpace(17, $rb_top_fh);                                                                          print $rb_top_fh "clk,\n";
  print $rb_top_fh "  ", "input";        printSpace(17, $rb_top_fh);                                                                          print $rb_top_fh "reset\n";
  ...
  print $rb_top_fh "  ", "wire "; printSpace(8, $rb_top_fh);  print $rb_top_fh "[11:0]"; printSpace(9-length("[11:0]"), $rb_top_fh); print $rb_top_fh "waddr".";\n";
  print $rb_top_fh "  ", "wire "; printSpace(8, $rb_top_fh);  print $rb_top_fh "[11:0]"; printSpace(9-length("[11:0]"), $rb_top_fh); print $rb_top_fh "raddr".";\n";
  print $rb_top_fh "  ", "wire "; printSpace(8, $rb_top_fh);  print $rb_top_fh "[31:0]"; printSpace(9-length("[31:0]"), $rb_top_fh); print $rb_top_fh "wdata".";\n";
  ...
  print $rb_top_fh "  ", "wire "; printSpace(17, $rb_top_fh);                                                                        print $rb_top_fh "wr_en".";\n";
  print $rb_top_fh "  ", "wire "; printSpace(17, $rb_top_fh);                                                                        print $rb_top_fh "rd_en".";\n";
  ...
  for (my $reg_index = 1; $reg_index <= $#reg_array; $reg_index++){
    ...
    for ( my $inst_index = 0; $inst_index < $inst_no; $inst_index++) {
      ...
      for ( my $field_index = 1; $field_index <= $#reg_field; $field_index++ ) {
        ...
        if ( lc($reg_field[$field_index]{'field_io'}) =~ /internal/ ) {
          print $rb_top_fh "  ", "wire ";
          printSpace(8, $rb_top_fh);
          print $rb_top_fh "$reg_field_width";
          printSpace(9-length($reg_field_width), $rb_top_fh);
          print $rb_top_fh "$signal_name".";\n";
...
#  rbModuleInstGen($rb_top_fh, $rb_derivative_reg_fh,  "rb_derivative_reg");
...
#------------------------------------------------------------------------#
  ...
  sub printHeader (@) {
    ...
    print $FileHandle "//    Project: dti_uart\n";
    print $FileHandle "//    Author: hautx0\n";
